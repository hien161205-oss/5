...
import { ArrowLeft, ShoppingCart, Star, ChevronRight as ChevronRightIcon } from 'lucide-react';
...
export default function CategoryPage() {
  ...
  const displayName = categoryNameMap[categoryId || ''] || 'Danh mục';
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-12">
              {filteredProducts.map((p, idx) => (
                ...
                >
                  <div className="relative aspect-[4/5] overflow-hidden rounded-3xl bg-brand-50 mb-6 shadow-md hover:shadow-2xl hover:-translate-y-2 transition-all duration-500 border border-brand-50/50">
                    <img 
                      src={p.imageUrl} 
                      alt={p.name}
                    ...
                    <button 
                      ...
                      className="absolute bottom-4 right-4 bg-white text-brand-500 p-4 rounded-full shadow-xl hover:bg-brand-500 hover:text-white transition-all transform translate-y-2 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 active:scale-95"
                    >
                  ...
                  <div className="space-y-2">
                    ...
                    <h3 className="text-base font-medium text-gray-900 group-hover:text-brand-600 transition-colors uppercase tracking-tight line-clamp-2 leading-tight">{p.name}</h3>
                    <p className="text-lg font-serif italic text-brand-500">{formatPrice(p.price)}</p>
                  </div>
                </motion.div>
              ))}
            </div>
