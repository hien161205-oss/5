...
import { Search, SlidersHorizontal, Plus, Zap, ChevronLeft, ChevronRight as ChevronRightIcon, Star } from 'lucide-react';
  ...
  return (
    <div className="space-y-12 pb-20">
        ...
        <div className="relative h-[30vh] md:h-[50vh] lg:h-[60vh] rounded-[3rem] overflow-hidden bg-brand-100 shadow-2xl shadow-brand-100/20">
          ...
          <div className="relative z-10 h-full flex flex-col justify-center px-12 md:px-20 space-y-4 max-w-2xl">
            <motion.span 
              ...
              className="text-xs md:text-sm uppercase tracking-[0.4em] text-white/90 font-black drop-shadow-md"
            >
            ...
            >
              3CE STYLENANDA <br /> <span className="italic font-light">BEST SELLERS</span>
            </motion.h1>
            <motion.p 
              initial={{ opacity: 0, x: -20 }}
              animate={{ opacity: 1, x: 0 }}
      ...
      <section className="max-w-7xl mx-auto px-6 mt-16 md:mt-24">
        <div className="bg-brand-50 rounded-[2rem] overflow-hidden shadow-xl shadow-brand-100/50 border border-brand-100">
          <div className="bg-gradient-to-r from-brand-600 via-blue-600 to-brand-600 py-4 px-8 flex items-center justify-center space-x-3">
            <Zap className="w-6 h-6 text-yellow-400 fill-current animate-pulse" />
            <h2 className="text-2xl font-black italic tracking-widest text-white uppercase">Flash Sale</h2>
            <Zap className="w-6 h-6 text-yellow-400 fill-current animate-pulse" />
          </div>
          ...
          <div className="p-8 space-y-8">
            <div className="flex flex-col md:flex-row items-center justify-between gap-6">
              <div className="flex items-center space-x-3">
                <span className="text-sm font-bold text-brand-600 uppercase tracking-widest">• Ưu đãi giới hạn</span>
              </div>
              
              <div className="flex items-center space-x-4">
                <span className="text-xs font-bold text-brand-400 uppercase tracking-widest">Thời gian còn lại:</span>
                <div className="flex space-x-2">
                  {[timeLeft.hours, timeLeft.minutes, timeLeft.seconds].map((val, i) => (
                    <div key={i} className="flex items-center">
                      <div className="bg-white border-2 border-brand-100 rounded-lg w-10 h-10 flex items-center justify-center font-bold text-brand-600 shadow-sm">
                        {val.toString().padStart(2, '0')}
                      </div>
                      {i < 2 && <span className="mx-1 font-bold text-brand-300">:</span>}
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
              {products.filter(p => p.onFlashSale).slice(0, 5).map((p) => (
                <div 
                  key={p.id} 
                  className="bg-white rounded-2xl p-4 border border-brand-50/50 shadow-md hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group cursor-pointer relative"
                  onClick={() => navigate(`/product/${p.id}`)}
                >
                    <div className="absolute top-2 right-2 z-10">
                      <span className="bg-brand-600 text-white text-[11px] font-black px-2 py-1 rounded-sm">-{p.flashSaleDiscount || 25}%</span>
                    </div>
                    <div className="aspect-square relative overflow-hidden rounded-xl mb-4">
                      <img src={p.imageUrl} alt={p.name} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" />
                    </div>
                    <div className="space-y-1">
                      <h4 className="text-sm font-medium text-gray-700 line-clamp-2 min-h-[2.5rem] leading-tight">{p.name}</h4>
                      <div className="flex flex-col">
                        <span className="text-brand-600 font-bold text-base">
                          {formatPrice(p.price * (1 - (p.flashSaleDiscount || 25) / 100))}
                        </span>
                        <span className="text-[11px] text-gray-300 line-through">{formatPrice(p.price)}</span>
                      </div>
                    </div>
                </div>
              ))}
              ...
              <div key={pageIdx} className="w-full flex-shrink-0 grid grid-cols-2 md:grid-cols-5 gap-6">
                {bestSellers.slice(pageIdx * itemsPerPage, (pageIdx + 1) * itemsPerPage).map((p) => (
                  <motion.div 
                    ...
                    onClick={() => navigate(`/product/${p.id}`)}
                  >
                    <div className="relative aspect-square w-full rounded-2xl overflow-hidden bg-brand-50 mb-4 shadow-md group-hover:shadow-2xl hover:-translate-y-2 transition-all duration-500">
                    ...
                    <div className="text-center space-y-1">
                      <h4 className="text-sm font-bold text-gray-800 line-clamp-1 uppercase tracking-tight">{p.name}</h4>
                      <p className="text-sm font-serif italic text-gray-500">{formatPrice(p.price)}</p>
                    </div>
                  </motion.div>
                ))}
              </div>
              ...
              <div key={pageIdx} className="w-full flex-shrink-0 grid grid-cols-2 md:grid-cols-5 gap-6">
                {latestProducts.slice(pageIdx * itemsPerPage, (pageIdx + 1) * itemsPerPage).map((p, idx) => (
                  <motion.div 
                    ...
                    onClick={() => navigate(`/product/${p.id}`)}
                  >
                    <div className="relative aspect-square overflow-hidden rounded-[2rem] bg-brand-50 mb-4 shadow-md group-hover:shadow-2xl hover:-translate-y-2 transition-all duration-500">
                    ...
                    <div className="text-center space-y-1">
                      <h3 className="text-sm font-bold text-gray-800 line-clamp-1 uppercase tracking-tight">{p.name}</h3>
                      <p className="text-sm font-serif italic text-brand-500">{formatPrice(p.price)}</p>
                    </div>
                  </motion.div>
                ))}
              </div>
