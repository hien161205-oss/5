...
function Overview() {
  ...
  const statCards = [
    ...
    { label: 'Đơn hàng', value: stats.orders, icon: ShoppingBag, color: 'text-blue-500', bg: 'bg-blue-50', path: '/admin/orders' },
    { label: 'Bài viết', value: stats.blogs, icon: FileText, color: 'text-brand-400', bg: 'bg-brand-50/50', path: '/admin/blogs' },
    { label: 'Cộng đồng', value: stats.users, icon: Users, color: 'text-purple-500', bg: 'bg-purple-50', path: '/admin/users' },
  ];
...
function CouponManagement() {
  ...
  const handleAddCoupon = async () => {
    try {
      ...
      await firestoreSetDoc(firestoreDoc(collection(db, 'coupons')), {
        ...newCoupon,
        usageCount: 0,
  ...
  return (
    <div className="space-y-12">
      <header>
        <h2 className="text-2xl font-serif">Mã giảm giá</h2>
        <p className="text-xs text-gray-500 mt-1">Chiến lược khuyến mãi và ưu đãi khách hàng.</p>
      </header>

      {isAdding && (
        <div className="bg-brand-50 p-8 rounded-[2.5rem] border border-brand-100 space-y-6">
          <h3 className="text-lg font-serif">Tạo mã giảm giá mới</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            <div className="space-y-2">
              <label className="text-[10px] uppercase font-bold tracking-widest text-brand-500 ml-4">Mã Code</label>
              <input 
                type="text" placeholder="Ví dụ: SUMMER2024"
                value={newCoupon.code}
                onChange={(e) => setNewCoupon({...newCoupon, code: e.target.value.toUpperCase()})}
                className="w-full bg-white rounded-2xl px-6 py-4 outline-none focus:ring-1 focus:ring-brand-200 uppercase font-mono"
              />
            </div>
            <div className="space-y-2">
              <label className="text-[10px] uppercase font-bold tracking-widest text-brand-500 ml-4">Loại giảm giá</label>
              <select 
            ...
            <div className="space-y-2">
              <label className="text-[10px] uppercase font-bold tracking-widest text-brand-500 ml-4">Giá trị giảm</label>
              <input 
                type="text"
                value={newCoupon.discountValue === 0 ? '' : newCoupon.discountValue}
                onChange={(e) => {
                  const val = e.target.value.replace(/[^0-9]/g, '').replace(/^0+(?!$)/, '');
                  setNewCoupon({...newCoupon, discountValue: val === '' ? 0 : Number(val)});
                }}
                className="w-full bg-white rounded-2xl px-6 py-4 outline-none focus:ring-1 focus:ring-brand-200"
              />
            </div>
            <div className="space-y-2">
              <label className="text-[10px] uppercase font-bold tracking-widest text-brand-500 ml-4">Hạn dùng</label>
              <input 
                type="date"
            ...
            <div className="space-y-2">
              <label className="text-[10px] uppercase font-bold tracking-widest text-brand-500 ml-4">Số lượng tối đa</label>
              <input 
                type="text"
                value={newCoupon.usageLimit === 0 ? '' : newCoupon.usageLimit}
                onChange={(e) => {
                  const val = e.target.value.replace(/[^0-9]/g, '').replace(/^0+(?!$)/, '');
                  setNewCoupon({...newCoupon, usageLimit: val === '' ? 0 : Number(val)});
                }}
                className="w-full bg-white rounded-2xl px-6 py-4 outline-none focus:ring-1 focus:ring-brand-200"
              />
            </div>
          ...
          <div className="flex space-x-4 pt-4">
            <button 
              ...
              className="bg-brand-500 text-white px-8 py-3 rounded-full text-xs font-bold uppercase tracking-widest shadow-lg shadow-brand-100"
            >
              Lưu mã mới
            </button>
            <button 
      ...
      <div className="bg-white rounded-[2.5rem] border border-brand-50 p-8 shadow-sm">
        <div className="flex justify-between items-center mb-8">
           <h4 className="text-[10px] uppercase font-black tracking-[0.3em] text-brand-300">Danh sách mã hiện có</h4>
           {!isAdding && (
             <button 
              onClick={() => setIsAdding(true)}
              className="text-brand-500 text-xs font-bold uppercase tracking-widest flex items-center space-x-2"
             >
               <Plus className="w-4 h-4" />
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm">
            <tbody className="divide-y divide-brand-50">
                  ...
                  <td className="py-6 pr-6">
                     <div className="flex items-center space-x-4">
                       ...
                       <div>
                         ...
                         <p className="text-[10px] text-gray-400 font-medium">Hạn dùng: {new Date(coupon.expiryDate).toLocaleDateString('vi-VN')}</p>
                       </div>
                     </div>
                  </td>
                  <td className="py-6 px-6">
                     <p className="text-[10px] uppercase font-bold tracking-widest text-gray-400">Giảm giá</p>
                     <p className="font-serif italic text-brand-600">
                       {coupon.discountType === 'percentage' ? `${coupon.discountValue}%` : formatPrice(coupon.discountValue)} 
                       {" "}trên tổng đơn
                     </p>
                  </td>
                  <td className="py-6 px-6">
                     <p className="text-[10px] uppercase font-bold tracking-widest text-gray-400">Số lượng</p>
                     <p className="text-sm font-bold text-gray-900">{coupon.usageCount || 0} / {coupon.usageLimit}</p>
                  </td>
                  <td className="py-6 pl-6 text-right">
                     <button 
                      onClick={() => handleDeleteCoupon(coupon.id)}
                      className="text-[10px] uppercase font-black text-gray-300 hover:text-red-500 transition-colors"
                     >
                       Xóa mã
                     </button>
                  </td>
              ...
              {coupons.length === 0 && (
                <tr>
                  <td colSpan={4} className="py-12 text-center text-gray-400 italic">Chưa có mã giảm giá nào được tạo.</td>
                </tr>
              )}
            </tbody>
          </table>
